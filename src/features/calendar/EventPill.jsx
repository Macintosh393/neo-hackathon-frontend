function getPillClasses(status, isCompromised, compromiseReason) {
	if (status === 'COMPLETED') {
	...
	if (isCompromised) {
		if (compromiseReason && compromiseReason.toLowerCase().includes('deadline violated')) {
			return 'border-rose-200/80 bg-gradient-to-r from-rose-50 to-rose-50/50 text-rose-700 hover:border-rose-300 hover:shadow-rose-100';
		}
...
function EventPill({ title, courseName, status, isCompromised, compromiseReason, onClick }) {
	...
	return (
		<Component
			...
			onClick={onClick}
			className={`neo-event-pill ${getPillClasses(status, isCompromised, compromiseReason)} ${isInteractive ? 'neo-event-pill-interactive' : ''}`}
		>
